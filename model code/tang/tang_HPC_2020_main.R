...
elbow <- function(start_position, direction, length)  {
  ...
  x <- start[1]
  y <- start[2]
  angle <- pi/2 + direction + pi/4
  l <- length*0.95
  turtle(c(x, y), l, angle)
  return(c(x2, y2)) # you should return your endpoint here.
}
...
# Question 27
draw_spiral <- function(start_position = c(-2, 3), direction= 0, length = 3)  {
  # clear any existing graphs and plot your graph within the R window
    start <- turtle(start_position, direction, length)
    x <- start[1]
    y <- start[2]
...
tree <- function(start_position, direction, length)  {
  ...
  x <- start[1]
  y <- start[2]
  angle1 <- direction + pi/4
  angle2 <- direction - pi/4
  l <- length*0.65
  tree(c(x, y), angle1, l)
  tree(c(x, y), angle2, l)
}
...
graphics.off()
plot(-4:4, -4:4, type = "n", xlab="", ylab="")
draw_tree <- function(start_position = c(0, -4), direction = -pi/2, length = 3)  {
  # clear any existing graphs and plot your graph within the R window
  start <- turtle(start_position, direction, length)
  x <- start[1]
  y <- start[2]
